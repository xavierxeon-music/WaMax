...
File::Init::Init(const Package::Info* info, Max::RefStructure& structure)
...
void File::Init::write(const Patch::Info& patchInfo)
{
   ...
   if (structure.header.patcherType == Max::RefStructure::PatchType::Standard) // delete file
   {
   ...
   else
   {
      ...
      if (structure.header.patcherType == Max::RefStructure::PatchType::Gui)
      {
      ...
      else if (structure.header.patcherType == Max::RefStructure::PatchType::Poly)
      {
         stream << "max objectfile " << patchInfo.name << " " << patchInfo.name << ";\n";
