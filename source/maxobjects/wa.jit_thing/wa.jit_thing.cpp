#include "wa.jit_thing.h"

class wa_jit_thing : public object<wa_jit_thing>, public matrix_operator<>
{
public:
...
private:
message<> jitclass_setup{this, "jitclass_setup", MIN_FUNCTION{t_class* c = args[0];

jit_class_addmethod(c, (method)wa_jit_thing_update_anim, "update_anim", max::A_CANT, 0);
jit_class_addinterface(c, jit_class_findbyname(gensym("jit_anim_animator")),
                       calcoffset(minwrap<wa_jit_thing>, m_min_object) + calcoffset(wa_jit_thing, animator), 0);
...
class_addmethod(c, (method)max_wa_jit_thing_int, "int", max::A_LONG, 0);
class_addmethod(c, (method)max_wa_jit_thing_bang, "bang", max::A_NOTHING, 0);
class_addmethod(c, (method)max_wa_jit_thing_assist, "assist", max::A_CANT, 0);
...
message<> setup{this, "setup", MIN_FUNCTION{animator = (t_object*)max::jit_object_new(gensym("jit_anim_animator"), maxobj());
// attr_addfilterset_proc(object_attr_get(animator, symbol("automatic")),
// (method)wa_jit_thing_automatic_attrfilter);
...
else if (!(classname() == "jit.mo.time") && !(classname() == "jit.time"))
{
   ...
   if (classname() == "jit.time.line")
      wa_jit_thing::function = functypes::line;
   else if (classname() == "jit.time.tri")
      wa_jit_thing::function = functypes::tri;
   else if (classname() == "jit.time.sin")
      wa_jit_thing::function = functypes::sin;
   else if (classname() == "jit.time.saw")
      wa_jit_thing::function = functypes::saw;
   else if (classname() == "jit.time.perlin")
      wa_jit_thing::function = functypes::perlin;
}
return {};
}
}
...
MIN_EXTERNAL(wa_jit_thing);

void wa_jit_thing_update_anim(t_object* job, t_atom* a)
{
   minwrap<wa_jit_thing>* self = (minwrap<wa_jit_thing>*)job;
